...
<template>
  <header
    class="sticky top-0 inset-x-0 -mt-12 flex flex-wrap sm:justify-start sm:flex-nowrap z-40 bg-white text-sm dark:bg-gray-800">
    <nav class="max-w-full w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between" aria-label="Global">
      ...
      <div id="navbar-image-1"
        ...
        <div data-hs-scrollspy="#scrollspy-1"
          class="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-end sm:mt-0 sm:ps-5">
          <a :class="{ 'text-indigo-600': currentSection === 'home' }"
            @click.prevent="navigateToSection('home')">Home</a>
          <a :class="{ 'text-indigo-600': currentSection === 'about' }"
            @click.prevent="navigateToSection('about')">About</a>
          <a :class="{ 'text-indigo-600': currentSection === 'work' }"
            @click.prevent="navigateToSection('work')">Work</a>
          <a :class="{ 'text-indigo-600': currentSection === 'blog' }"
            @click.prevent="navigateToSection('blog')">Blog</a>
      ...
      <Dialog as="div" @close="closeModal" class="fixed inset-0 overflow-y-auto z-50">
        <div class="flex min-h-full items-center justify-center p-4 text-center">
          <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
            ...
            <DialogPanel
              class="w-full md:w-[800px] transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all z-50 dark:bg-gray-800">
              <LoginComponent />
            </DialogPanel>
          </TransitionChild>
        </div>
  ...
  <div id="about" class="mt-12 section relative h-screen overflow-hidden">
    ...
    <p class="text-lg text-center mt-10">We are a small team working on the next-generation business solution. Updates
      coming soon.</p>
  </div>
